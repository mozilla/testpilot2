...
var Study = Class({
  ...
  unpersist: function() { /* loop over prefs? */ },
  go:  function go(){
    let my = this.config;
    console.log("firing study:",my.id);  
    let that = this;
  ...
  askInstall: function(success){
    console.log("askInstall");
    let F = "askinstall";
    let my = this.config;
    if (my.flags[F]) {return success !== undefined && success(this);}
    ...
    if (my.studytype == "simplesurvey") { 
        return do_survey(this);
    }
    var askbanner = experiment_install_consent(this); // this is tangled
    
    // TODO:  handle surveys....
    
    /*
    here is where we should handle decided / no signal from the thing.
    ...
    var intervalid = timers.setInterval(function() {
      ...
      if (attempts > nags.length) { this.setStatus(MISSED); return False}
      if (Date.now() < nextattempt ) { this.unwait(); };
  ...
  askUpload: function(success){
    ...
    var askbanner = experiment_upload_consent(this); // this signals status back
...
var experiment_install_consent = function(study){
    var decided = false;
    var tpb = tpbanner({
        ...
        B.no({callback:  function() { decided=true; emit(study,'status','REFUSED')}}), 
        B.yes({callback:  function() { decided=false; emit(study,'status','ACCEPTED')}})]
    })
    ...
    */
    ...
    tpb.events.on("AlertClose",
        function () {
            console.log("banner closing. decided?", decided );         
            (!decided) && emit(study,'status','REFUSED')}
    );
    return tpb;
};

var experiment_upload_consent = function(study){
    var decided= false;
    var tpb = tpbanner({
...
var finish_survey = function(survey){
    flags = survey.config.flags;
    ['askinstall','install','done','askupload','upload'].forEach(
    function(F) {flags[F] = true})
    survey.persist();
    switchtab(survey.config.url);
}


var do_survey = function(survey,success,failure) {
    ...
    */
    ...
    var tpb = tpbanner({
        ...
        buttons: [
            ...
            B['no thanks']({callback:  function() {setStatus(survey,REFUSED)}})
        ]
    });
...
// TODO, sorry this is outside the object.  too much global stuff
var start_experiment = function(experiment) {
    ...
    */
    ...
    (experiment.config['observe'] || []).forEach(
        function (topic) {
...
var main = function(options,callback) {
    ...
    setup_and_run();
    var check_interval = timers.setInterval(function() {
        setup_and_run();
    },tp_hygeine_interval);
};
