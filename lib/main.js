...
var revive = function(id,startbranch){
    ...
    let fromdisk = Branch(startbranch + diskkey);
...
var Study = Class({
  ...
  uninstall:  function uninstall(success){
    ...
    // uninstall extensions, only if no-one else is using them
  ...
  start_experiment: function() {
    ...
    */
    ...
    if (!this.installed_observers) {this.installed_observers = new Collection()}
    var obscoll = this.installed_observers;
    ...
    if (my.eventselectors ) {
        ...
        this.pagemods = [ EventSelector(x) for each (x in my.eventselectors) ];
    }
