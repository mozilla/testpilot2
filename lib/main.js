...
const {banner,doorhanger,nbButtons:B} = require("moz-ui");
...
// to do, turn this into a promise...
var get_consent = function(experiment,callback){
    banner('yo we doing this thing!');
    return true;
};
...
// this is one way of setting up an experiment... is it right?
...
var do_survey = function(survey,callback) {
    ...
    var surveyor = require("panel").Panel({
      width: 400,
      height: 400,
      contentURL: data.url('simple_survey.html')
    });
    let searchbox = window.document.getElementById("searchbar");
    console.log(searchbox,"found");
    surveyor.show(searchbox);
    /*
...
var do_experiment = function(experiment,callback) {
    ...
    /*var confirm_start = require("panel").Panel({
      width: 400,
      height: 400,
      contentURL: data.url('confirm_experiment_start.html')
    });
    //confirm_start.show();
    */
    var will_run = get_consent();
...
var start_experiment = function(id,fn,window,callback){
    let experiment_tracker = new windowUtils.WindowTracker({
        onTrack: function (window) {
            console.log("new window tracked, should cram experiements in there");
            fn(window)
        }
...
var optin = require("panel").Panel({
  width: 180,
  height: 180,
  contentURL: data.url('optin.html')
...
//require('addon-kit/addon-page');
let uri = require('self').data.url('index.html');
let tab = openTab(window, uri);

console.log("B.yes:",JSON.stringify(B.yes()));

banner("here is a message");
banner("here is a message",'',0,null,[B.yes(),B.no()]);


// remember the password gives doorhanger!
