...
var sample_experiment_code = function(window) {
   ...
   * 1. listen for mouse-driven command events on the main menu bar: */
      ...
      mainMenuBar.on("command", function(evt) {
        ...
        record({menuId:menuid, menuItemId:menuItemId, etype:"mouse"},"sample_exp1");
        },
        true);
  ...
  mainCommandSet.on("command", function(evt) {
    ...
    if (tag.tagName == "menuitem") {
      ...
      record({tag: tag.command?tag.command:tag.id, etype:"key shortcut"},"sample_exp1");
    }},
    true);
