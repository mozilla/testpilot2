...
// change this and remove file when https://github.com/mozilla/addon-sdk/pull/525/ lands
...
let ISSETTINGUP = false;  // TODO, this is a cheap lock.
...
var parse_studies = function(json,callback) {
    ...
    let create_update_revive = function(s,defaults) {
        ...
        if (cur.config.status === undefined) {
            ...
            if (revived && In(cur.config.status,[S.ASKINSTALL,S.COLLECTING,S.ASKUPLOAD])){
...
PageMod({
  ...
  onAttach: function onAttach(worker) {
    ...
    var uiPageObserver = function(subject, data){
      if (subject.message == "setstatus" && worker) {
        worker.port.emit("populatestudies", tpstudies);
      }
    };

    observer.add("testpilot", uiPageObserver);

    worker.on('detach', function () {
      observer.remove("testpilot", uiPageObserver);
    });
...
var tpdebugmode = function tpdebugmode(thisaddonsandbox){
    let nags = [1,4,10];
    //if (! myprefs['indexurl']) {
        myprefs['indexurl'] = data.url("example/example.json") // TODO, what is right here?
    //};
